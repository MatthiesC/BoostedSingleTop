...
void copyAndRenameHists(TString pt_bin, TString channel)
{
  TFile* final_file = TFile::Open("input/"+channel+"/theta-input_PT-"+pt_bin+".root", "RECREATE"); // my output file
  ...
  for(int i = 0; i < fileNames.size(); i++)
      ...
      for(int j = 0; j < fileNames.at(i).size(); j++)
	{
	  ...
	  TH1F *input_histo = (TH1F*)input_file_->Get("MVA_1t/"+pt_bin+"_mvaD_rebin01");
...
void writeInputHists()
{
  ...
  for(int bin = 0; bin < pt_bins.size(); ++bin)
    {
      copyAndRenameHists(pt_bins.at(bin),"Muon");
      copyAndRenameHists(pt_bins.at(bin),"Electron");
    }
}
